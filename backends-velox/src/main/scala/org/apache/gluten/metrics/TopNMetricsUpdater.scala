...
// Velox lowers TopN to a bounded-heap `TopNNode`, which never spills. Hence, unlike
// SortMetricsUpdater, no spill-related metrics are collected here.
class TopNMetricsUpdater(val metrics: Map[String, SQLMetric]) extends MetricsUpdater {

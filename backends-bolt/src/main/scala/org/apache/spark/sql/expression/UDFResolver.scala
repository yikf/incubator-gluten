...
object UDFResolver extends Logging {
  ...
  def resolveUdfConf(sparkConf: SparkConf, isDriver: Boolean): Unit = {
    val udfLibPaths = if (isDriver) {
      sparkConf
        .getOption(BoltBackendSettings.GLUTEN_BOLT_DRIVER_UDF_LIB_PATHS)
        .orElse(sparkConf.getOption(BoltBackendSettings.GLUTEN_BOLT_UDF_LIB_PATHS))
    } else {
      sparkConf.getOption(BoltBackendSettings.GLUTEN_BOLT_UDF_LIB_PATHS)
    }
    ...
    udfLibPaths match {
      case Some(paths) =>
        ...
        sparkConf.set(
          BoltBackendSettings.GLUTEN_BOLT_INTERNAL_UDF_LIB_PATHS,
  ...
  private def checkAllowTypeConversion: Boolean = {
    SQLConf.get
      .getConfString(BoltBackendSettings.GLUTEN_BOLT_UDF_ALLOW_TYPE_CONVERSION, "false")
      .toBoolean
  }

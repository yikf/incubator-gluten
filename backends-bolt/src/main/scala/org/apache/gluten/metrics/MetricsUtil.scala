...
object MetricsUtil extends Logging {
  ...
  private def operatorMetricFromJson(node: JsonNode): OperatorMetrics = {
    ...
    metrics.remainingFilterTime = customMetricSum(node, "totalRemainingFilterTime")
  ...
  def genMetricsUpdatingFunction(
    ...
    child.session.sparkContext.register(accumulator, "bolt task stats")
  ...
  def updateTransformerMetricsInternal(
      ...
      operatorIdx: JLong,
      metrics: Metrics,
      nativeMetrics: JArrayList[OperatorMetrics],
      metricsIdx: Int,
      joinParamsMap: JMap[JLong, JoinParams],
  ...
  def genMetricsUpdatingFunction(
    ...
    imetrics =>
      ...
      val numNativeMetrics = metrics.numMetrics

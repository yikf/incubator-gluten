  ...
  override protected def doGeneratorValidate(
    ...
    if (!supportsGenerate(generator)) {
      ValidationResult.failed(
        s"Bolt backend does not support this generator: ${generator.getClass.getSimpleName}" +
          s", outer: $outer")
    } else {
...
object GenerateExecTransformer {
  def supportsGenerate(generator: Generator): Boolean = {
    generator match {
      case _: ExplodeBase | _: JsonTuple =>
        true
      // TODO: need to support inline and stack
      case _: Inline | _: Stack =>
        false
      case _ =>
        false
    }
      ...
      generate.generator match {
        case PosExplode(_) =>
          ...
          if (generate.outer) {
            ...
            // Reorder the generatorOutput to match the output from the Unnest operator in Bolt.

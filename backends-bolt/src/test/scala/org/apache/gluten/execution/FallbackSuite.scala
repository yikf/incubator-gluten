        ...
        Seq("true", "false").foreach {
          parquetUseColumnNames =>
            Seq("true", "false").foreach {
              orcUseColumnNames =>
                withSQLConf(
                  BoltConfig.PARQUET_USE_COLUMN_NAMES.key -> parquetUseColumnNames,
                  BoltConfig.ORC_USE_COLUMN_NAMES.key -> orcUseColumnNames
                ) {
                  withTable("test") {
                    spark
                      .range(100)
        ...
        events =>
          ...
          assert(
            ...
            s"Expected no GlutenPlanFallbackEvent for vanilla Spark execution, " +
              s"but got ${events.size} event(s). First event fallback reasons: " +
              s"${events.headOption.map(_.fallbackNodeToReason)}"
          )
      }
    }
  }

...
case class BoltIcebergWriteToDataSourceV2Exec(
    query: SparkPlan,
    refreshCache: () => Unit,
    write: Write,
...
object BoltIcebergWriteToDataSourceV2Exec {
  ...
  def apply(original: WriteToDataSourceV2Exec): Option[BoltIcebergWriteToDataSourceV2Exec] = {
    extractOuterWrite(original.batchWrite)
      .filter(supportsWrite)
      .map {
        write =>
          BoltIcebergWriteToDataSourceV2Exec(
            original.query,

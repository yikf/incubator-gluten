...
abstract class AbstractIcebergWriteExec extends IcebergWriteExec {
  ...
  private def createIcebergDataWriteFactory(schema: StructType): IcebergDataWriteFactory = {
    ...
    Seq(
      AbstractIcebergWriteExec.PARQUET_PAGE_SIZE_BYTES_KEY -> getParquetPageSizeBytes,
      AbstractIcebergWriteExec.MAX_TARGET_FILE_SIZE_KEY -> getTargetFileSizeBytes
    ).foreach {
      case (key, value) =>
...
object AbstractIcebergWriteExec {
  private val PARQUET_PAGE_SIZE_BYTES_KEY =
    "spark.gluten.sql.columnar.backend.bolt.parquet.pageSizeBytes"
  private val MAX_TARGET_FILE_SIZE_KEY =
    "spark.gluten.sql.columnar.backend.bolt.parquetMaxTargetFileSize"
}

...
struct SplitInfo {
  enum class LeafType {
    /// A streaming node that accepts iterator-backed end-of-input notification.
    SPLIT_AWARE_STREAM = 0,
    /// A table scan node that accepts file scan splits.
    TABLE_SCAN = 1,
    /// A leaf node that doesn't rely on splits.
    TRIVIAL_LEAF = 2
  };

  /// The type of the associated Bolt leaf query plan node.

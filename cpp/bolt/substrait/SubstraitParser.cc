...
TypePtr SubstraitParser::parseType(const ::substrait::Type& substraitType, bool asLowerCase) {
  switch (substraitType.kind_case()) {
    ...
    case ::substrait::Type::KindCase::kNothing:
      return UNKNOWN();
    default:
      BOLT_NYI("Parsing for Substrait type not supported: {}", substraitType.DebugString());
  }
}
...
void SubstraitParser::parseColumnTypes(
  ...
  if (columnsTypes.size() == 0) {
    ...
    BOLT_CHECK_EQ(columnsTypes.size(), namedStruct.names().size(), "Wrong size for column types and column names.");
  }
  ...
  for (const auto& columnType : columnsTypes) {
    switch (columnType) {
      ...
      //   break;
      default:
        BOLT_FAIL("Unspecified column type.");
    }
  }
  return;
}
...
bool SubstraitParser::parseReferenceSegment(
  ...
  switch (typeCase) {
    ...
    default:
      BOLT_NYI("Substrait conversion not supported for ReferenceSegment '{}'", std::to_string(typeCase));
  }
}
...
int64_t SubstraitParser::configLongValueInOptimization(
  ...
  if (extension.optimization_size() > 0) {
    ...
    if (pos != std::string::npos && endPos != std::string::npos) {
      std::string valueStr = msg.value().substr(startPos, endPos - startPos);
      try {
...
template <>
bytedance::bolt::UnknownValue gluten::SubstraitParser::getLiteralValue(const substrait::Expression_Literal& literal) {
  return UnknownValue();
}
...
StringView SubstraitParser::getLiteralValue(const ::substrait::Expression::Literal& literal) {
  if (literal.has_string()) {
    ...
    BOLT_FAIL("Unexpected string or binary literal");
  }
}

const std::unordered_map<std::string, std::string>& SubstraitParser::substraitBoltFunctionMap(bool useIcuRegex) {
  if (useIcuRegex) {
    return substraitBoltFunctionMapWithICU_;
  } else {
    return substraitBoltFunctionMap_;
  }
}

std::unordered_map<std::string, std::string> SubstraitParser::substraitBoltFunctionMap_ = {
...
std::unordered_map<std::string, std::string> SubstraitParser::substraitBoltFunctionMapWithICU_ = [] {
  std::unordered_map<std::string, std::string> icuFunc = {
      {"regexp", "icu_regexp"},
      {"regexp_extract", "icu_regexp_extract"},
      {"regexp_extract_all", "icu_regexp_extract_all"},

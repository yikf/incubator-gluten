...
class Substrait2BoltPlanConversionTest : public exec::test::HiveConnectorTestBase {
 protected:
  std::vector<std::shared_ptr<bytedance::bolt::connector::ConnectorSplit>> makeSplits(
    ...
    std::vector<std::shared_ptr<bytedance::bolt::connector::ConnectorSplit>> splits;
  ...
  std::shared_ptr<BoltPlanConverter> planConverter_ =
      std::make_shared<BoltPlanConverter>(std::vector<std::shared_ptr<ResultIterator>>(), pool(), boltCfg_.get());
};
...
TEST_F(Substrait2BoltPlanConversionTest, q6) {
  ...
  std::shared_ptr<const bytedance::bolt::config::ConfigBase> config{
      std::make_shared<bytedance::bolt::config::ConfigBase>(std::move(hiveConfig))};
...
// TODO sync bolt and restore it
// TEST_F(Substrait2BoltPlanConversionTest, ifthenTest) {
